...
class tintuc{
    ...
    public function get_tentintucA(){
        $query = "SELECT * FROM tbl_tentintuc,tbl_tintuc WHERE tbl_tentintuc.tintuc_id = tbl_tintuc.tintuc_id 
        AND tbl_tentintuc.tintuc_id =  '$_GET[tentintuc_id]' ORDER BY tbl_tentintuc.tentintuc_id DESC";
        $result = $this ->db->select($query);
        return $result;
    }

    public function get_tentintuc($tentintuc_id){
